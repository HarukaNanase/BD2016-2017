									...
									<div class="w-clearfix w-col w-col-3">
										<div class="sign-in signup">
											<div class="w-clearfix w-form">
												<?php
													if ($_POST['type'] == "espaco") {
														echo "<form class='form signup' action='rem_loc_esp.php' data-name='remove_local' id='wf-form-remove_local' method='post' name='wf-form-remove_local'>";
													} else {
														echo "<form class='form signup' action='rem_loc_p4.php' data-name='remove_local' id='wf-form-remove_local' method='post' name='wf-form-remove_local'>";
													}
												?>
												<select class="text-field w-select" data-name="tipo" id="tipo" name="type" readonly><option value="<?=$_POST['type']?>"><?=$_POST['type']?></option></select>
													...
													<select class="text-field w-select" data-name="codigo" id="codigo" name="codigo">
														<optgroup label="Espacos Existentes">
														<?php
															$type = $_POST['type'];
															require 'bd_init.php';
															$sql    = "SELECT espaco.codigo FROM espaco WHERE espaco.morada='{$_POST['morada']}';";
															$result = $db->query($sql);
